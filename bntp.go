...
func WithSyncPeriod(period time.Duration) Option {
	return func(ts *TimeSync) {
		if period > 0 {
			if period < time.Hour {
				log.Println("[WARN] ntp refresh period suggest to be >= 1 hour")
			}
...
func SetTimeSync(options ...Option) (*TimeSync, error) {
	...
	if err != nil {
		log.Printf("[WARN] get cache dir fail, use default: %v\n", err)
		cacheDir = os.TempDir()
	}
	curLang := LangCode
	if curLang == LangNone {
...
func GetTimeSync() *TimeSync {
	if timeSyncer == nil {
		ts, err := SetTimeSync()
		if err != nil {
			log.Printf("[WARN] ntp initialization failed, using local time: %v\n", err)
		}
		return ts
	}
	return timeSyncer
...
func (ts *TimeSync) setOptions(options ...Option) error {
	...
	if err != nil {
		log.Printf("[WARN] failed to load time offset: %v\n", err)
	}
...
func (ts *TimeSync) loadOffsetFromFile() (bool, error) {
	...
	log.Printf("[INFO] restored ntp, offset_ms=%d, from=%s\n", record.Offset, time.Unix(record.Timestamp, 0).Format(time.RFC3339))
	return true, nil
}
...
func (ts *TimeSync) refresh() error {
	...
	if err = ts.saveOffsetToFile(); err != nil {
		log.Printf("[WARN] failed to save time offset: %v\n", err)
	}
	return nil
}
...
func (ts *TimeSync) loopSync() {
	...
	for {
		...
		case <-time.After(syncPeriod):
			if err := ts.Refresh(); err != nil {
				log.Printf("[WARN] time sync refresh failed: %v\n", err)
			}
		}
	}
}
...
func (ts *TimeSync) syncTime() error {
	...
	for _, server := range servers {
		...
		if err != nil {
			lastErr = err
			log.Printf("[WARN] ntp query failed, server=%s: %v\n", server, err)
			continue // 尝试下一个服务器
		}
		...
		log.Printf("[INFO] ntp sync successful, server=%s, offset_ms=%d\n", server, offsetMs)
		return nil // 同步成功
	}
